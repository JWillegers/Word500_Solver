...
'''
TUI with lookup table, entropy and suggestions
'''
def game_with_lookup_table():
    # asking for difficulty
    input_accepted = False
    while not input_accepted:
        difficulty = input('Which difficult do you want to play on? (easy/medium/hard)')
        ...
        if not input_accepted:
            print('Input not accepted, please try again')
    # prepare game
    accepted_words, lookup_table = prepare_game.get_table(difficulty, False)
    word_found = False
    turn_counter = 1
    print_top_20(lookup_table)
    # playing the game
    while not word_found and turn_counter <= 8:
        input_accepted = False
        # ask for word and colors
        while not input_accepted:
            guess = input(
                'What did you guess? answer format: word [space] amount green [space] amount yellow [space] amount red')
            split = guess.split(' ')
            try:
                if not len(split[0]) == 5:
                    print('please enter a 5 letter word. Try again')
        ...
        # process guess
        lookup_table = solver.process_guess(split[0], int(split[1]), int(split[2]), int(split[3]), lookup_table)
        turn_counter += 1
        print_top_20(lookup_table)
    if word_found:
        print('Congrats, you won')
...
def print_top_20(lookup_table):
    lookup_table.sort_values(by='entropy', ascending=False)
    print(lookup_table.head(10))


if __name__ == '__main__':
    #start_game()
    game_with_lookup_table()
